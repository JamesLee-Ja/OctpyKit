...
class ExcelToPDFApp(TkinterDnD.Tk):
    def __init__(self):
        ...
        self.files = []
        # ページ向きモード: 0=全縦, 1=全横, 2=縦横交互(縦始), 3=縦横交互(横始)
    ...
    def convert_one_file(self, excel_path):
        ...
        try:
            ...
            for idx, ws in enumerate(wb.Worksheets):
                ...
                # ユーザー選択に応じてページ向きを決定
                if mode == 0:
                    orientation = 1                 # すべて縦向き
                elif mode == 1:
                    orientation = 2                 # すべて横向き
                elif mode == 2:
                    orientation = 1 if idx % 2 == 0 else 2  # 縦→横→縦…
                else:
                    orientation = 2 if idx % 2 == 0 else 1  # 横→縦→横…
                ps.Orientation = orientation

                # 縦横交互を確実にするため、1シート=1ページに強制する
                # FitToPagesTall=Falseにすると複数ページが生成され交互が崩れる
                ps.Zoom = False                     # 必須（FitToPages系を有効にするためFalseに）
                ps.FitToPagesWide = 1               # 横方向: 1ページに収める
                ps.FitToPagesTall = 1               # 縦方向: 1ページに収める（交互を保証）

                # 既存の印刷範囲をクリアし、使用済み領域全体を印刷範囲に設定
                # → 内容が切れず、かつページ全体にフィットする
                ps.PrintArea = ""
                try:
                    used = ws.UsedRange
                    if used is not None:
                        # 図形・グラフが使用済み領域より外にある場合も考慮して拡張
                    ...
                    ps.PrintArea = ""               # 失敗時はExcelの自動検出に委ねる

                # 極小余白 + センタリングでコンテンツを最大化
                ps.LeftMargin = 5
                ps.RightMargin = 5
                ps.TopMargin = 10
                ps.BottomMargin = 10
